...
export const DuckDBRenderer: FC<RendererComponent> = ({repoId, refId, path, fileExtension }) => {
    let initialQuery = `SELECT * FROM READ_PARQUET('jzfs://${repoId}/${refId}/${path}') LIMIT 20`;
    if (fileExtension === 'csv') {
        initialQuery = `SELECT *  FROM READ_CSV('jzfs://${repoId}/${refId}/${path}', AUTO_DETECT = TRUE) LIMIT 20`
    } else if (fileExtension === 'tsv') {
        initialQuery = `SELECT *  FROM READ_CSV('jzfs://${repoId}/${refId}/${path}', DELIM='\t', AUTO_DETECT=TRUE) LIMIT 20`
    }
